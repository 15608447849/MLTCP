...
//        ("收到: \n"+message);
    ...
    public void close() {
        socketClient.getSession().getOperation().writeString("CLOSE","UTF-8");
...
//                long pos = 0;
                while( isUsing && ( len = randomAccessFile.read(bytes) )> 0){
//                    pos += len;
//                    (Thread.currentThread(),flag," "+curFile," 单次量 : "+ len," 进度: "+ String.format("%.2f",((double)pos/ curFile.getFileLength())));
                    op.writeBytes(bytes,0,len);
                    endUsingTime = System.currentTimeMillis();
                }
            }else{
            ...
            if (isUsing) (Thread.currentThread(),flag," 结束 ", curFile.getFullPath()," ",
